...
def entropymax(chi2):
    ...
    LM_LHS_tf = tf.pow(-tf.reduce_sum(poly_coef_tf*np.arange(0,poly_order[0],1).reshape((poly_order[0],1)) ) - ReFr,2)
...
for i in range(0,20):
    chi[i], H[i] = entropymax(0.2*i)
    plt.plot(chi[i],H[i],'x')
    plt.pause(0.1)


np.savetxt('chi2.out', chi, delimiter=',')
np.savetxt('entropy.out', H, delimiter=',')
plt.plot(chi,H)
