...
def TFFoptimizechi2(initial_weights,n_points,poly_order,gjpower,chi2):
    #SGD parameters
    SGD_rate = 1e-8
    SGD_noise = SGD_rate*1e4

    #weighting for lagrange multipliers
    LM_boundary_weight = 1000
    LM_positive_weight = 1000
    LM_shape_factor = 10000
    ...
    if module_testing_on:
        ...
        poly_coef_np[0] = 0 + np.random.rand(1)
        poly_coef_np[1] = 2.0 + np.random.rand(1)
        poly_coef_np[2] = -1 + np.random.rand(1)
        poly_coef_np[3] = 0 + np.random.rand(1)
...
for i in range(0,100):
    chi2 = 1.05 + i*0.002
    H, chi2new = TFFoptimizechi2(initial_weights,n_points,poly_order,gjpower,chi2)
    plt.plot(chi2new,H,'x')
    plt.pause(0.1)
